...
% for each gamma, ones and zeros saved in one row of inputs matrix
for j=1:length(gama)
    for i=1:1000
         x=rand;
        if x<p11(j)
...
%Eigenvalues and eig.vectors of covariance matrices above
ShowEigen(Cov_num_1)
ShowEigen(Cov_num_2)
ShowEigen(Cov_num_3)
ShowEigen(Cov_num_4)
...
for j=1:length(gama)
    ...
    for i=1:999
    wR(j,i+1)=wR(j,i)+dt * (cs_an*wR(j,i)+cd_an(j)*wL(j,i));

    wL(j,i+1)=wL(j,i)+dt *(cd_an(j)*wR(j,i)+cs_an*wL(j,i));

    wplus(j,i+1)= wplus(j,i)+ dt *(wplus(j,i)*(cs_an+cd_an(j)));
...
wR_new=zeros(5,1000);
wL_new=zeros(5,1000);
initials_wR=[0.05 0.10 0.15 0.20 0.25];
initials_wL=[0.25 0.20 0.15 0.10 0.05];
...
for j=1:length(initials_wR)
    ...
    wR_new(j,i+1)=wR_new(j,i)+dt * (cs_an*wR_new(j,i)+cd_an(3)*wL_new(j,i));

    wL_new(j,i+1)=wL_new(j,i)+dt *(cd_an(3)*wR_new(j,i)+cs_an*wL_new(j,i));


    
    wplus_new(j,i+1)= wplus_new(j,i)+ dt *(wplus_new(j,i)*(cs_an+cd_an(3)));
...
xlabel('w_L','FontSize',25);
ylabel('w_R','FontSize',25);
legend('I.P=0.05,0.25','I.P= 0.10,0.20','I.P=0.15,0.15','I.P=0.20,0.10','I.P=0.25,0.05');
hold off
...
new_intl_wR=0.25*rand(1,n);
new_intl_wL=0.25*rand(1,n);
wR_n=zeros(n,1000);
wL_n=zeros(n,1000);
...
for j=1:n
    ...
    wR_n(j,i+1)=wR_n(j,i)+dt * (cs_an*wR_n(j,i)+cd_an(3)*wL_n(j,i));

    wL_n(j,i+1)=wL_n(j,i)+dt *(cd_an(3)*wR_n(j,i)+cs_an*wL_n(j,i));
