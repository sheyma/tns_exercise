...
cum_r=zeros(1,2*N);
cum_b=zeros(1,2*N);
beta=2;                     % constant for choice variability
eps=0.01;
...
for i=1:N
    ...
    a=rand;
        ...
        if a<P_r(i)
        mr(i+1)=mr(i)+eps*(1-P_r(i))*(r_r1-r_ave1);
        mb(i+1)=mb(i)-eps*P_b(i)*(r_r1-r_ave1);
        Nr=Nr+1;
        
%         number_r(rt+1)=number_r(rt)+1;
        else
        mr(i+1)=mr(i)-eps*P_r(i)*(r_b1-r_ave1);
        mb(i+1)=mb(i)+eps*(1-P_b(i))*(r_b1-r_ave1);
        Nb=Nb+1;
...
%     if i<(2*N) && rt<1000
    if a<P_r(i) 
        mr(i+1)=mr(i)+eps*(1-P_r(i))*(r_r2-r_ave2);
        mb(i+1)=mb(i)-eps*P_b(i)*(r_r2-r_ave2);
        Nr=Nr+1;
%         number_r(rt+1)=number_r(rt)+1;
%         number_b(rt+1)=number_b(rt);
    else
        mr(i+1)=mr(i)-eps*P_r(i)*(r_b2-r_ave2);
        mb(i+1)=mb(i)+eps*(1-P_b(i))*(r_b2-r_ave2);
        Nb=Nb+1;
